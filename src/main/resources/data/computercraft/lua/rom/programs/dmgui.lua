...
root:addLabel()
    :setText("Attached Readers")
    :setPosition(2, 2)
    :setSize((screenX / 2) - 6, 1)
    :setBackground(colors.white)

root:addLabel()
    :setText("Information")
    :setPosition((screenX / 2) - 2, 2)
    :setSize((screenX / 2) + 3, 1)
    :setBackground(colors.white)

local readerSide = root:addTable()
                        :setPosition(2, 4)
                        :setSize((screenX / 2) - 6, screenY - 5)
                        :setColumns({
                            {name = "Reader"},
                        })

local manageSide = root:addContainer()
                       :setPosition((screenX / 2) - 2, 4)
                       :setSize((screenX / 2) + 3, screenY - 5)
...
parallel.waitForAll(
        ...
        function()
            readerSide:onRowSelect(function()
                manageSide:addLabel()
                          :setText("yay")
            end)
        end
